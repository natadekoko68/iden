...
sns.boxplot(data=df_concated, x="Sample Name", y="Relative Quantity",
            order=["WT", "WT-Ecoli", "WT-PBS", "C", "C-Ecoli", "C-PBS"], color='white')
sns.swarmplot(data=df_concated, x="Sample Name", y="Relative Quantity",
              order=["WT", "WT-Ecoli", "WT-PBS", "C", "C-Ecoli", "C-PBS"], palette='Set2')
plt.title("各サンプルのmRNA量")
for i in range(1, 6):
    plt.text(i,
             df_concated[df_concated["Sample Name"] == list(["WT", "WT-Ecoli", "WT-PBS", "C", "C-Ecoli", "C-PBS"])[i]][
